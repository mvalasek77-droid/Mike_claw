...
|------|---------|----------------|------|
| **L1 — Headless engine** | `xcodebuild test` on macOS / CI (`WatchfighterTests`) | All deterministic combat logic: damage, combos, AI, ladder flow, boss rule, difficulty, mode rules. **The bulk of bug-finding.** | Cheap, fast, repeatable |
...
**seeded RNG** (`WatchfighterEngine(seed:)`) and debug hooks (`debugSetPlayer`,
...
### 2.1 Core combat (`WatchfighterEngine.performAttack`, `applyHit`)
...
- `FFScript` (intro, midpoint, pit, beforeBoss, all `chapter(_:)` beats) —
  `GameScreen.swift`
- Banners/callouts in `WatchfighterEngine` (`showBanner`, signature moves,
...
existing `WATCHFIGHTER_DEMO=1` flag:
...
Add a macOS CI job: `xcodegen generate` → `xcodebuild test -scheme Watchfighter
