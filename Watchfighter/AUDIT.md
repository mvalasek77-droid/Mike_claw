# Watchfighter — Audit & Improvements (movement + bugs)
...
- **Smooth strike envelope** (`actionStrike`): attacks/hits now thrust out fast
  ...
  `setAction`). `GameModels.swift`, `WatchfighterEngine.swift`, `WatchfighterCanvas.swift`.
- **Quicker player tracking:** Crown/touch follow response `13.5 → 17.5` — snappier
  control, still lerp-smooth. `WatchfighterEngine.swift`.
