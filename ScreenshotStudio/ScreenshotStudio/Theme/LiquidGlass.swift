...
/// look on iOS 17–25.
enum LiquidGlass {
    static let accent = Color(red: 0.38, green: 0.46, blue: 1.0)
    static let accentSecondary = Color(red: 0.95, green: 0.39, blue: 0.73)
    static let success = Color(red: 0.26, green: 0.84, blue: 0.58)
    ...
    static let lightBaseGradient = LinearGradient(
        colors: [
            ...
            Color(red: 0.99, green: 0.95, blue: 1.00)
        ],
        startPoint: .topLeading,
        endPoint: .bottomTrailing
    )
...
struct LiquidGlassBackground: View {
    ...
    var body: some View {
        ZStack {
            ...
            // Reduce Motion so we don't burn battery on a TimelineView the
    ...
    private func drawOrb(in ctx: GraphicsContext, size: CGSize, t: TimeInterval, hueShift: Double) {
        ...
        let color = Color(hue: 0.66 + hueShift, saturation: 0.82, brightness: 0.96).opacity(0.22)
...
// Wrapper so the file still compiles on Xcode versions without the iOS 26
// SDK — the modifier becomes a no-op rather than a build failure.
