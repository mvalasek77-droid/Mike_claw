#!/usr/bin/env bash
# Smoke test for the consumables Worker. Runs the public + authed happy paths
# against a running instance (local `wrangler dev` or a deployed URL).
#
# Usage:
...
curl -fsS "${AUTH[@]}" "${BASE_URL}/balance?userId=${USER_ID}" | tee /dev/stderr | grep -q '"credits"'

echo; echo "→ POST /consume with no balance should 402"
code=$(curl -s -o /dev/null -w '%{http_code}' "${AUTH[@]}" \
  -H 'Content-Type: application/json' \
  -d "{\"userId\":\"${USER_ID}\",\"credits\":10,\"idempotencyKey\":\"smoke-1\"}" \
  "${BASE_URL}/consume")
[ "$code" = "402" ] && echo "  ok: 402 insufficient credits" || { echo "  FAIL: got $code"; exit 1; }

echo; echo "→ POST /checkout (needs a valid STRIPE_SECRET_KEY to succeed)"
curl -s "${AUTH[@]}" -H 'Content-Type: application/json' \
  -d "{\"packId\":\"credits_100\",\"userId\":\"${USER_ID}\"}" \
