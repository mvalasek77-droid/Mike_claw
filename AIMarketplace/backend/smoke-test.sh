#!/usr/bin/env bash
#
# Staging smoke test for the AI Marketplace payout Worker.
#
...
#   → ledger. Shows you exactly what each endpoint returns so you can confirm
#   the sale notification + top-up math without touching real money.
#
# It never sends or stores secrets anywhere — you pass them as env vars and
# they live only in your shell for this run.
#
# ── Usage ─────────────────────────────────────────────────────────────────
#   export STAGING_URL="https://aimarketplace-payout-staging.<you>.workers.dev"
...
#   # (payouts enabled). Create one via the app in staging, or reuse the one
#   # from your earlier test. Find it in Stripe Dashboard (test mode) →
#   # Connect → Accounts, or in the app's Payout Setup status card.
...
hr "4. Transfer ${AMOUNT} (the 'sale')  POST /payouts/transfer"
echo "On success: ledger records it + you get the 💰 sale email."
echo "On insufficient funds: 502 + the 🚨 NSF email fires."
SALE_ID="smoke-$(date +%s)"
curl -s "${auth[@]}" -H "Content-Type: application/json" \
  -d "{\"account_id\":\"${ACCOUNT}\",\"amount_usd\":${AMOUNT},\"title_id\":\"${SALE_ID}\",\"idempotency_key\":\"sale_${SALE_ID}\",\"memo\":\"smoke test sale\"}" \
...
hr "Done"
echo "Check ${OPERATOR_EMAIL:-mvalasek77@gmail.com} for the sale (or NSF) email."
