...
private struct ScreenplayReelSurface: View {
    ...
    private func sceneBlock(number: Int, text: String, minutes: Int) -> some View {
        ...
        return SceneBlockView(number: number, text: text, minutes: minutes, videoURL: videoURL)
    }
...
/// One scene as a chapter card. Owns its own AVPlayer via @StateObject so
/// scrolling through scenes doesn't allocate a fresh player per body pass —
/// fixes the rapid-scroll memory leak the audit flagged.
private struct SceneBlockView: View {
    let number: Int
    let text: String
    let minutes: Int
    let videoURL: URL?
    @StateObject private var playback = AVPlaybackModel()
    ...
    var body: some View {
        ...
        .onAppear {
            if let url = videoURL, !playback.hasMedia { playback.load(url: url, previewLimit: nil) }
        }
