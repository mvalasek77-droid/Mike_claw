...
enum FighterArchetype: CaseIterable, Equatable, Sendable {
    ...
    case dracula
    /// The Hell demon met in the Pit after a ladder fall (`WatchsmashState.pitActive`).
    ...
    /// throw — see `WatchsmashEngine.performAttack`.
...
struct WatchsmashState: Equatable, Sendable {
    ...
    /// `WatchsmashEngine.resolveRoundIfNeeded`/`startNextRound`.
