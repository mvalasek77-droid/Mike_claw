...
struct WatchsmashCanvas: View {
    var state: WatchsmashState
    var date: Date
    ...
    private func drawBackground(in context: inout GraphicsContext, size: CGSize) {
        ...
        if state.pitActive || state.finisherTimer > 0 {
    ...
    private func drawGuard(in context: inout GraphicsContext, anchor: CGPoint, fighter: DuelFighter, size: CGSize) {
        ...
        context.fill(Path(ellipseIn: shield), with: .color(Color.watchsmashMint.opacity(0.12)))
        context.stroke(Path(ellipseIn: shield), with: .color(Color.watchsmashMint.opacity(0.48)), lineWidth: 1.4)
    }
        ...
        for strike in state.strikes {
            ...
            if strike.kind == .blood {
                ...
                for index in 0..<9 {
                    ...
                    context.fill(Path(ellipseIn: drop), with: .color(Color.watchsmashRed.opacity(Double(1 - progress) * 0.80)))
                }
                continue
            }
    ...
    private func drawFinisherEffect(in context: inout GraphicsContext, center: CGPoint, radius: CGFloat, progress: CGFloat, kind: StrikeKind, size: CGSize) {
        ...
        let red = Color.watchsmashRed.opacity(fade * 0.82)
        let gold = Color.watchsmashGold.opacity(fade * 0.70)
        ...
        for index in 0..<9 {
            ...
            context.stroke(ray, with: .color(Color.watchsmashRed.opacity(fade * 0.30)), lineWidth: max(1, 3.2 * (1 - progress)))
        }
        ...
        for index in 0..<chunkCount {
            ...
            if index.isMultiple(of: 2) {
                ...
                context.fill(Path(ellipseIn: CGRect(x: trail.x - chunkSize * 0.18, y: trail.y - chunkSize * 0.18, width: chunkSize * 0.36, height: chunkSize * 0.36)), with: .color(Color.watchsmashRed.opacity(fade * 0.6)))
            }
        }
        ...
        if kind == .headPop {
            ...
            context.fill(Path(ellipseIn: head), with: .color(Color.watchsmashGold.opacity(fade * 0.9)))
            context.stroke(Path(ellipseIn: head), with: .color(Color.watchsmashRed.opacity(fade * 0.85)), lineWidth: max(1, radius * 0.10))
            ...
            context.stroke(streak, with: .color(Color.watchsmashRed.opacity(fade * 0.55)), lineWidth: max(2, radius * 0.16))
            ...
            context.fill(Path(roundedRect: upperArm, cornerRadius: radius * 0.16), with: .color(Color.watchsmashRed.opacity(fade * 0.82)))
            context.fill(Path(roundedRect: forearm, cornerRadius: radius * 0.16), with: .color(Color.watchsmashRed.opacity(fade * 0.78)))
            context.fill(Path(ellipseIn: CGRect(x: forearm.midX - radius * 0.22, y: forearm.maxY - radius * 0.06, width: radius * 0.44, height: radius * 0.40)), with: .color(Color.watchsmashGold.opacity(fade * 0.78)))
            var streak = Path()
            streak.move(to: center)
            streak.addLine(to: CGPoint(x: upperArm.midX, y: upperArm.minY))
            context.stroke(streak, with: .color(Color.watchsmashRed.opacity(fade * 0.5)), lineWidth: max(2, radius * 0.14))
        } else {
            context.fill(Path(ellipseIn: CGRect(x: center.x - radius, y: center.y - radius, width: radius * 2, height: radius * 2)), with: .color(Color.watchsmashRed.opacity(fade * 0.28)))
            context.stroke(Path(ellipseIn: CGRect(x: center.x - radius * 1.4, y: center.y - radius * 1.4, width: radius * 2.8, height: radius * 2.8)), with: .color(Color.watchsmashGold.opacity(fade * 0.65)), lineWidth: max(2, radius * 0.15))
        }
    }
    ...
    private func drawArenaSetPieces(in context: inout GraphicsContext, size: CGSize, palette: ArenaPalette) {
        ...
        case .dragonAlley:
            for index in 0..<5 {
                ...
                context.fill(Path(roundedRect: lantern, cornerRadius: 3), with: .color(Color.watchsmashRed.opacity(0.55)))
                context.stroke(Path(roundedRect: lantern, cornerRadius: 3), with: .color(Color.watchsmashGold.opacity(0.55)), lineWidth: 1)
            }
        case .sunPier:
            let water = CGRect(x: 0, y: size.height * 0.46, width: size.width, height: size.height * 0.12)
            context.fill(Path(water), with: .linearGradient(Gradient(colors: [Color.watchsmashMint.opacity(0.20), .black.opacity(0)]), startPoint: water.origin, endPoint: CGPoint(x: 0, y: water.maxY)))
            context.fill(Path(roundedRect: CGRect(x: size.width * 0.76, y: size.height * 0.34, width: size.width * 0.12, height: size.height * 0.09), cornerRadius: 3), with: .color(Color.watchsmashRed.opacity(0.42)))
        case .runwayTerminal, .redCarpet:
            ...
            context.fill(Path(roundedRect: CGRect(x: size.width * 0.18, y: size.height * 0.57, width: size.width * 0.64, height: size.height * 0.04), cornerRadius: 2), with: .color(Color.watchsmashRed.opacity(0.24)))
        case .blackoutBase:
            for index in 0..<4 {
                let x = size.width * (0.18 + CGFloat(index) * 0.20)
                context.stroke(Path(ellipseIn: CGRect(x: x, y: size.height * 0.30, width: 18, height: 7)), with: .color(Color.watchsmashMint.opacity(0.35)), lineWidth: 1.5)
            }
        case .launchFoundry:
            var rocket = Path()
        ...
        case .goldRally:
            for index in 0..<4 {
                ...
                context.fill(Path(roundedRect: rect, cornerRadius: 2), with: .color(Color.watchsmashGold.opacity(0.28)))
            }
        case .icePalace:
            for index in 0..<5 {
        ...
        case .obsidianThrone:
            ...
            context.stroke(Path(roundedRect: throne, cornerRadius: 5), with: .color(Color.watchsmashGold.opacity(0.35)), lineWidth: 1.2)
        case .millionRoom:
            for index in 0..<3 {
                let x = size.width * (0.22 + CGFloat(index) * 0.25)
                context.fill(Path(roundedRect: CGRect(x: x, y: size.height * 0.30, width: 18, height: 52), cornerRadius: 8), with: .color(Color.watchsmashRed.opacity(0.28)))
                context.stroke(Path(roundedRect: CGRect(x: x, y: size.height * 0.30, width: 18, height: 52), cornerRadius: 8), with: .color(Color.watchsmashGold.opacity(0.22)), lineWidth: 1)
            }
        default:
            break
        }
    ...
    private func drawHellLandscape(in context: inout GraphicsContext, size: CGSize) {
        ...
        context.stroke(cliffs, with: .color(Color.watchsmashRed.opacity(0.58)), lineWidth: 1.2)
        ...
        for index in 0..<11 {
            ...
            context.fill(flame, with: .linearGradient(
                Gradient(colors: [Color.watchsmashGold, Color(red: 1.0, green: 0.08, blue: 0.01), .black.opacity(0.12)]),
        ...
        context.stroke(lava, with: .color(Color.watchsmashRed.opacity(0.62)), lineWidth: 5.4)
        context.stroke(lava, with: .color(Color.watchsmashGold.opacity(0.90)), lineWidth: 2.2)
    }
    ...
    private func arenaPalette(_ chapter: StoryChapter) -> ArenaPalette {
        ...
        case .cinderGate:
            return ArenaPalette(
                ...
                line: Color.watchsmashGold,
                accent: Color.watchsmashGold,
        ...
        case .neonRooftop:
            return ArenaPalette(
                ...
                line: Color.watchsmashMint,
                accent: Color(red: 1.0, green: 0.30, blue: 0.58),
                fire: Color.watchsmashMint,
        ...
        case .obsidianThrone:
            return ArenaPalette(
                ...
                line: Color.watchsmashRed,
                accent: Color.watchsmashGold,
                fire: Color.watchsmashRed,
    ...
    private func drawActionMotion(in context: inout GraphicsContext, anchor: CGPoint, fighter: DuelFighter, size: CGSize) {
        ...
        let color = fighter.action == .special ? Color.watchsmashMint : Color.watchsmashGold
        ...
        case .jumpKick:
            ...
            context.stroke(arc, with: .color(Color.watchsmashGold.opacity(0.56)), lineWidth: max(2.2, unit * 0.11))
        case .throwAttack:
            var grab = Path()
            ...
            context.stroke(grab, with: .color(Color.watchsmashGold.opacity(0.62)), lineWidth: max(2, unit * 0.10))
        case .projectile:
    ...
    private func drawProceduralFighter(in context: inout GraphicsContext, fighter: DuelFighter, anchor: CGPoint, size: CGSize) {
        ...
        case .mara:
            stripe(-0.76, 0.12, 1.44, 0.16, color: Color.watchsmashGold.opacity(0.88))
            var sash = Path()
            sash.move(to: pt(-0.78, 0.28))
            sash.addQuadCurve(to: pt(0.46, 1.12), control: pt(-0.18, 0.78))
            fighterContext.stroke(sash, with: .color(Color.watchsmashRed.opacity(0.72)), lineWidth: max(1.2, unit * 0.10))
            var blade = Path()
        ...
        case .lennox:
            stripe(-0.56, 0.72, 1.12, 0.18, color: Color.watchsmashGold.opacity(0.92))
            stripe(-0.70, -1.88, 1.06, 0.12, color: Color.watchsmashRed.opacity(0.88))
            var dragon = Path()
            ...
            fighterContext.stroke(dragon, with: .color(Color.watchsmashRed.opacity(0.82)), lineWidth: max(1.0, unit * 0.065))
        case .sunny:
            stripe(-0.74, -0.42, 1.48, 0.12, color: .white.opacity(0.62))
            stripe(-0.58, 0.32, 1.16, 0.12, color: .white.opacity(0.52))
            stripe(-0.30, -2.32, 0.62, 0.18, color: Color.watchsmashGold.opacity(0.86))
        ...
        case .specter:
            stripe(-0.42, -1.80, 0.84, 0.18, color: Color.watchsmashMint.opacity(0.86))
            ...
            stripe(0.34, 0.88, 0.28, 0.62, color: Color.watchsmashMint.opacity(0.48))
        case .cosmo:
            ...
            fighterContext.fill(Path(ellipseIn: rect(-1.00, 0.48, 0.28, 0.34)), with: .color(Color.watchsmashGold.opacity(0.44)))
            fighterContext.fill(Path(ellipseIn: rect(0.74, 0.48, 0.28, 0.34)), with: .color(Color.watchsmashGold.opacity(0.44)))
        case .brass:
            ...
            stripe(-0.12, -0.98, 0.24, 1.02, color: Color.watchsmashRed)
            fighterContext.fill(Path(ellipseIn: rect(1.66 + punch, -0.72, 0.20, 0.20)), with: .color(Color.watchsmashGold.opacity(0.88)))
        case .volkov:
        ...
        case .zara:
            ...
            fighterContext.stroke(scarf, with: .color(Color.watchsmashGold.opacity(0.72)), lineWidth: max(1.4, unit * 0.12))
        ...
        case .cage:
            stripe(-0.70, 0.82, 1.40, 0.18, color: Color.watchsmashRed.opacity(0.92))
        ...
        case .titan:
            fighterContext.stroke(Path(ellipseIn: rect(-1.34, -0.16, 0.70, 0.60)), with: .color(Color.watchsmashGold), lineWidth: max(1.8, unit * 0.065))
            fighterContext.stroke(Path(ellipseIn: rect(0.74 + punch, -0.86, 0.74, 0.64)), with: .color(Color.watchsmashGold), lineWidth: max(1.8, unit * 0.065))
            stripe(-0.58, -0.62, 1.16, 0.20, color: Color.watchsmashGold.opacity(0.50))
            stripe(-0.62, 0.54, 1.24, 0.18, color: Color.watchsmashRed.opacity(0.54))
        case .dracula:
            var cape = Path()
