...
final class StoryboardFlowTests: XCTestCase {
    ...
    func testFullTournamentStoryboardHasEverySceneAndNoSoftLock() {
        var engine = WatchsmashEngine(seed: 0xC0FFEE)
        let bot = AutoPlayDriver()

        var lines: [String] = ["# Watchsmash — autoplay storyboard", ""]
        var sceneCount = 0
    ...
    private func writeArtifactIfRequested(_ markdown: String) {
        guard let dir = ProcessInfo.processInfo.environment["WATCHSMASH_ARTIFACT_DIR"] else { return }
    ...
    private func finishTitanWithMillionShot(_ engine: inout WatchsmashEngine) {
        engine.debugSetPlayer(x: 0.30)
    ...
    private func forceOpponentWin(_ engine: inout WatchsmashEngine) {
