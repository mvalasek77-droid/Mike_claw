    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 18) {
                ...
                HStack(spacing: 12) {
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        ...
                        if let floor = woman.startingBid {
                            Text("Floor \(Money.full(floor))").font(.dynamicScaled(12, weight: .semibold, relativeTo: .caption1))
                                .foregroundStyle(Theme.gold)
                        } else {
                            Text("No floor — open bidding").font(.dynamicScaled(12, relativeTo: .caption1)).foregroundStyle(Theme.inkFaint)
                        }
                    }
                    Spacer()
                }
                ...
                if let prompt = promptContext {
                    VStack(alignment: .leading, spacing: 4) {
                        Label("Your bid will mention this answer", systemImage: "quote.opening")
                            .font(.dynamicScaled(10, weight: .bold, design: .rounded, relativeTo: .caption2)).tracking(1)
                            .foregroundStyle(Theme.rose)
                        Text(prompt.question).font(.dynamicScaled(11, weight: .semibold, relativeTo: .caption2)).foregroundStyle(Theme.inkSoft)
                        Text(prompt.answer).font(.dynamicScaled(15, weight: .bold, design: .serif, relativeTo: .subheadline)).foregroundStyle(Theme.ink)
                ...
                // Big amount display.
                VStack(spacing: 4) {
                    Text("Your bid").font(.dynamicScaled(11, weight: .bold, design: .rounded, relativeTo: .caption2))
                        .tracking(1).foregroundStyle(Theme.inkFaint)
                    Text(Money.full(amount))
                        .font(.dynamicScaled(46, weight: .heavy, design: .rounded, relativeTo: .largeTitle))
                    ...
                    if amount >= Bid.masterpieceBid && store.role == .man && store.me.archetype == .trillionaire {
                        Label("Masterpiece-eligible — pay it & get confirmed", systemImage: "rosette")
                            .font(.dynamicScaled(12, weight: .bold, relativeTo: .caption1)).foregroundStyle(Theme.rose)
                    }
                }
                ...
                VStack(alignment: .leading, spacing: 6) {
                    Text("ADD A NOTE").font(.dynamicScaled(10, weight: .bold, design: .rounded, relativeTo: .caption2))
                        .tracking(1).foregroundStyle(Theme.inkFaint)
                    TextField("", text: $note,
                ...
                if atFreeLimit {
                    VStack(spacing: 8) {
                        Label("You've used all \(AuctionStore.freeActiveBidLimit) free live bids.",
                            ...
                            .font(.dynamicScaled(13, weight: .bold, relativeTo: .footnote)).foregroundStyle(Theme.warning)
                ...
                Text("Spend your bid on the date — the meal, the drinks, the night. She keeps the receipts and confirms it after. Never wire money or send a personal deposit; the app has no way to send money to another user, by design.")
                    .font(.dynamicScaled(11, relativeTo: .caption2)).foregroundStyle(Theme.inkFaint).multilineTextAlignment(.center)
                Spacer(minLength: 20)
            }
            .screenPadding()
        }
        ...
        Button { Haptics.selection(); insure.toggle() } label: {
            HStack(spacing: 12) {
                Image(systemName: insure ? "shield.checkerboard" : "shield")
                    .font(.dynamicScaled(20, weight: .bold, relativeTo: .title3))
                    .foregroundStyle(insure ? Theme.verify : Theme.inkSoft)
                VStack(alignment: .leading, spacing: 2) {
                    Text("Bid Insurance").font(.dynamicScaled(15, weight: .heavy, design: .serif, relativeTo: .subheadline))
                        .foregroundStyle(Theme.ink)
    ...
    private var gildToggle: some View {
        Button { Haptics.selection(); gild.toggle() } label: {
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    Text("Gild this bid").font(.dynamicScaled(15, weight: .heavy, design: .serif, relativeTo: .subheadline))
                        .foregroundStyle(Theme.ink)
