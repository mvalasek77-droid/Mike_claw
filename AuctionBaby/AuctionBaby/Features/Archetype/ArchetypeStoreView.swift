        ...
        GlassCard(tint: Theme.gold) {
            HStack {
                VStack(alignment: .leading, spacing: 6) {
                    Text("Your rating").font(.dynamicScaled(11, weight: .bold, design: .rounded, relativeTo: .caption2))
            ...
            GlassSurface(corner: Theme.cornerL,
                ...
                HStack(spacing: 14) {
                    ...
                    VStack(alignment: .leading, spacing: 3) {
                        HStack(spacing: 6) {
                            Text(tier.title).font(.dynamicScaled(16, weight: .heavy, design: .serif, relativeTo: .callout))
                                .foregroundStyle(Theme.ink)
                            if tier == .trillionaire {
                                Image(systemName: "rosette").font(.dynamicScaled(11, relativeTo: .caption2)).foregroundStyle(Theme.rose)
                            }
                        }
                        Text(tier.blurb).font(.dynamicScaled(12, relativeTo: .caption1)).foregroundStyle(Theme.inkSoft)
                    ...
                    VStack(alignment: .trailing, spacing: 4) {
                        switch tier.purchase {
                        case .free:
                            Text("Free").font(.dynamicScaled(16, weight: .heavy, design: .rounded, relativeTo: .callout))
                                .foregroundStyle(Theme.ink)
                        case .money:
                            Text(owned ? "Owned" : priceLabel)
                                .font(.dynamicScaled(16, weight: .heavy, design: .rounded, relativeTo: .callout))
                                .foregroundStyle(owned ? Theme.success
                                                 : tier.usesPrestigeStyle ? Theme.gold : Theme.ink)
                                .lineLimit(1).minimumScaleFactor(0.7)
                        }
                        if current {
                            Text(pending ? "PENDING" : "ACTIVE").font(.dynamicScaled(9, weight: .heavy, design: .rounded, relativeTo: .caption2))
