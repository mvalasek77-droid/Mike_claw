            ...
            ScrollView {
                VStack(spacing: 14) {
                    GlassCard(tint: Theme.verify) {
                        HStack(spacing: 12) {
                            Image(systemName: "shield.lefthalf.filled").font(.dynamicScaled(26, weight: .bold, relativeTo: .title1))
                    ...
                    ForEach(tips, id: \.1) { tip in
                        GlassSurface(corner: Theme.cornerL) {
                            HStack(alignment: .top, spacing: 12) {
                                Image(systemName: tip.0).font(.dynamicScaled(16, weight: .bold, relativeTo: .callout))
                                    .foregroundStyle(Theme.gold).frame(width: 30)
                                VStack(alignment: .leading, spacing: 3) {
                                    Text(tip.1).font(.dynamicScaled(14, weight: .bold, relativeTo: .footnote)).foregroundStyle(Theme.ink)
                                    Text(tip.2).font(.dynamicScaled(13, relativeTo: .footnote)).foregroundStyle(Theme.inkSoft)
                        ...
                        Link(destination: url) {
                            GlassSurface(corner: Theme.cornerL, tint: Theme.gold) {
                                HStack(alignment: .top, spacing: 12) {
                                    ...
                                    VStack(alignment: .leading, spacing: 3) {
                                        Text("Report a bug")
                                            .font(.dynamicScaled(14, weight: .bold, relativeTo: .footnote)).foregroundStyle(Theme.ink)
            ...
            ScrollView {
                VStack(spacing: 10) {
                    Text("Report \(profile.name)")
                        .font(.dynamicScaled(18, weight: .heavy, design: .serif, relativeTo: .body)).foregroundStyle(Theme.ink)
                        .padding(.top, 8)
                    Text("They'll be removed from your floor and won't be able to reach you. Reports are reviewed by our team.")
                        .font(.dynamicScaled(13, relativeTo: .footnote)).foregroundStyle(Theme.inkSoft).multilineTextAlignment(.center)
                    ...
                    ForEach(reasons, id: \.self) { reason in
                        Button {
                            ...
                            HStack {
                                Text(reason).font(.dynamicScaled(15, weight: .semibold, relativeTo: .subheadline)).foregroundStyle(Theme.ink)
                                Spacer()
    ...
    private func blockedCard(_ row: BlockedRow) -> some View {
        GlassSurface(corner: Theme.cornerL) {
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    Text(row.name).font(.dynamicScaled(14, weight: .heavy, design: .serif, relativeTo: .footnote))
                        .foregroundStyle(Theme.ink)
                    if let reason = row.reason, !reason.isEmpty {
                        Text(reason).font(.dynamicScaled(11, relativeTo: .caption2)).foregroundStyle(Theme.inkFaint).lineLimit(1)
                    }
                }
                Spacer()
                Button {
