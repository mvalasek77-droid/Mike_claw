...
struct AdminView: View {
        ...
        GlassCard(title: "Platform · live", icon: "waveform.path.ecg", tint: Theme.gold) {
            if let s = stats {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    Text(err).font(.dynamicScaled(12, weight: .semibold, relativeTo: .caption1)).foregroundStyle(Theme.danger)
            ...
            GlassSurface(corner: Theme.cornerL) {
                HStack(spacing: 12) {
                    Image(systemName: "banknote.fill")
                        .font(.dynamicScaled(16, weight: .semibold, relativeTo: .callout))
                        .foregroundStyle(Theme.success)
                        .frame(width: 44, height: 44)
                        .background(Circle().fill(Theme.success.opacity(0.14)))
                    VStack(alignment: .leading, spacing: 2) {
                        Text("Money & backend").font(.dynamicScaled(15, weight: .heavy, design: .serif, relativeTo: .subheadline))
        ...
        Button { sheet = .realUsers } label: {
            GlassSurface(corner: Theme.cornerL) {
                HStack(spacing: 12) {
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        Text("Signed-in users").font(.dynamicScaled(15, weight: .heavy, design: .serif, relativeTo: .subheadline))
                            .foregroundStyle(Theme.ink)
        ...
        Button { sheet = .audit } label: {
            GlassSurface(corner: Theme.cornerL) {
                HStack(spacing: 12) {
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        Text("Audit trail").font(.dynamicScaled(15, weight: .heavy, design: .serif, relativeTo: .subheadline))
                            .foregroundStyle(Theme.ink)
        ...
        Button { sheet = .moderation } label: {
            GlassSurface(corner: Theme.cornerL) {
                HStack(spacing: 12) {
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        Text("Moderation queue").font(.dynamicScaled(15, weight: .heavy, design: .serif, relativeTo: .subheadline))
                            .foregroundStyle(Theme.ink)
        ...
        Button { sheet = .editMe } label: {
            GlassSurface(corner: Theme.cornerL) {
                HStack(spacing: 12) {
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        Text("Your account").font(.dynamicScaled(12, weight: .bold, design: .rounded, relativeTo: .caption1))
                            .foregroundStyle(Theme.inkFaint)
    ...
    private func rosterSection(title: String, empty: String, people: [Profile],
        ...
        .padding(.top, 4)
        if people.isEmpty {
            Text(empty).font(.dynamicScaled(13, relativeTo: .footnote)).foregroundStyle(Theme.inkFaint)
                .frame(maxWidth: .infinity, alignment: .leading)
        } else {
        ...
        GlassSurface(corner: Theme.cornerL) {
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 3) {
                    HStack(spacing: 6) {
                        Text(profile.name).font(.dynamicScaled(15, weight: .heavy, design: .serif, relativeTo: .subheadline))
                            .foregroundStyle(Theme.ink)
                        if profile.verified { VerifiedBadge(size: 12) }
                        if profile.isCopycat { CopycatTag(compact: true) }
                    }
                    Text("\(profile.age) · \(profile.location)")
                        .font(.dynamicScaled(12, relativeTo: .caption1)).foregroundStyle(Theme.inkSoft).lineLimit(1)
                ...
                Button(action: onEdit) {
                    Image(systemName: "pencil").font(.dynamicScaled(14, weight: .bold, relativeTo: .footnote))
                        .foregroundStyle(Theme.gold)
                ...
                Button(action: onDelete) {
                    Image(systemName: "trash.fill").font(.dynamicScaled(15, weight: .bold, relativeTo: .subheadline))
