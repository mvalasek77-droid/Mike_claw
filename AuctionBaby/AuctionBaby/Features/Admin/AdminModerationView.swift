        ...
        GlassSurface(corner: Theme.cornerL) {
            VStack(alignment: .leading, spacing: 8) {
                HStack(alignment: .top) {
                    VStack(alignment: .leading, spacing: 2) {
                        ...
                        Text(r.targetId.prefix(8) + "…")
                            .font(.dynamicScaled(13, weight: .heavy, design: .monospaced, relativeTo: .footnote))
                ...
                if let ctx = r.context, !ctx.isEmpty {
                    Text(ctx).font(.dynamicScaled(12, relativeTo: .caption1))
                ...
                HStack(spacing: 6) {
                    Text("From").font(.dynamicScaled(9, weight: .heavy, design: .rounded, relativeTo: .caption2))
                    ...
                    if let by = r.resolvedBy, !by.isEmpty, r.status != "open" {
                        Text("· resolved by \(by)")
                            .font(.dynamicScaled(10, relativeTo: .caption2))
                            .foregroundStyle(Theme.inkFaint)
                    }
                }
                if r.status == "open" {
