...
| Reserve the date (booking fee) | Real-world Stripe fee, bidder picks a tier ($10/$15/$25/$50/$100), platform-kept, opens hosted Checkout | Full tier picker shown; "Demo: reserve free" marks the date reserved with no charge and no Stripe account needed. A ✓ Reserved seal then appears on the match. |
...
7. **Status tab** → every rating (Good Guy $4.99 → Trillionaire $9,999.99)
   is a real non-consumable IAP; **Demo Mode marks all eight as owned**, so
   tapping one shows "Wear this badge" and equips it free — no charge is
   ever made in Demo Mode. Switch between several to see the badge and the
   Auction Credit "Status" factor move. (Gavels can't buy status by design;
   they're for Gilded Bids, Bid Insurance and streak freezes.)
