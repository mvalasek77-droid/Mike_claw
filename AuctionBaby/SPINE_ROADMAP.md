...
- **2026-07-28** — Slice 7 shipped (auto-refresh on push: PushService.PushEvent enum + onEvent hook parses type/bidId/matchId from both foreground and tap paths; app root routes bid.received → refreshRemoteInbox, bid.accepted → refreshRemoteMatches, message.received → refreshRemoteMatch(matchId:), verified → refreshMe). Pushes now update the underlying data, not just the banner.
- **2026-07-28** — Slice 8 shipped (DOB gate on profile writes + reads: handleUpsertMyProfile refuses when users.date_of_birth is null; handleFloor and handleGetUserProfile filter/hide null-DOB accounts). Combined with the /me/dob 18+ enforcement from slice 1, no unverified-age profile can ever be floor-visible.
