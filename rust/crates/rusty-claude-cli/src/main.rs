...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match parse_args(&args)? {
        ...
        CliAction::Serve { port } => run_serve(port)?,
...
fn parse_args(args: &[String]) -> Result<CliAction, String> {
    ...
    match rest[0].as_str() {
        ...
        "serve" => {
            let port = if let Some(port_str) = rest.get(1) {
                port_str.parse::<u16>().map_err(|_| format!("invalid port number: {port_str}"))?
            } else {
                runtime::server::DEFAULT_PORT
            };
...
fn render_help_topic(topic: LocalHelpTopic) -> String {
  ...
  Related          claw doctor · claw status"
            .to_string(),
        LocalHelpTopic::Serve => "Serve
  Usage            claw serve [port]
  Purpose          start the CodeGenie HTTP server for iPhone app build requests
  Default port      8765
  Endpoints         GET /api/health  — health + version
                   POST /api/build  — submit a new build job
                   GET  /api/build/:id/status — poll job status
                   GET  /api/build/:id/stream — SSE event stream (stub)
  Related          claw doctor · claw status"
            .to_string(),
    }
}
    ...
    fn new(
        ...
        let client = match detect_provider_kind(&resolved_model) {
            ...
            | ProviderKind::AWSBedrock => {
                ...
                // `OpenAiCompatConfig` (openai / xai / dashscope / groq / etc.).
                // That reads the correct API-key env var and BASE_URL
                // override internally, so this one call covers all
...
fn run_serve(port: u16) -> Result<(), Box<dyn std::error::Error>> {
    ...
    rt.block_on(runtime::server::serve(port))
