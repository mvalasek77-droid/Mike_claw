...
const MODEL_REGISTRY: &[(&str, ProviderMetadata)] = &[
    ...
    (
        "ai21",
        ProviderMetadata {
            provider: ProviderKind::AI21,
            auth_env: "AI21_API_KEY",
            base_url_env: "AI21_BASE_URL",
            default_base_url: openai_compat::DEFAULT_AI21_BASE_URL,
        },
    ),
    // ── Ollama (local) ────────────────────────────────────────────────────
    (
    ...
    MODEL_REGISTRY
        ...
        .find_map(|(alias, metadata)| {
            (*alias == lower).then_some(match metadata.provider {
                ...
                ProviderKind::Perplexity => match *alias {
                    "sonar" => "sonar",
                    _ => trimmed,
                },
                ...
                ProviderKind::Together => match *alias {
                    "together" => "together",
                    _ => trimmed,
                },
                ProviderKind::AI21 => match *alias {
                    "ai21" => "ai21",
                    _ => trimmed,
                },
                ...
                // Catch-all for new provider kinds (Cloudflare, AWS, etc.)
                // that use bare model names — pass through unchanged.
                _ => trimmed,
            })
        })
...
pub fn metadata_for_model(model: &str) -> Option<ProviderMetadata> {
    ...
    // These prefixes work with any model name after the slash, routing to the
    // correct provider based on the prefix rather than the model name.
    ...
    if canonical.starts_with("perplexity/") {
        return Some(ProviderMetadata {
            provider: ProviderKind::Perplexity,
            auth_env: "PERPLEXITY_API_KEY",
            base_url_env: "PERPLEXITY_BASE_URL",
    ...
    if canonical.starts_with("cohere/") {
        return Some(ProviderMetadata {
            provider: ProviderKind::Cohere,
            auth_env: "COHERE_API_KEY",
            base_url_env: "COHERE_BASE_URL",
    ...
    if canonical.starts_with("aws/") || canonical.starts_with("bedrock/") {
        return Some(ProviderMetadata {
            provider: ProviderKind::AWSBedrock,
            auth_env: "AWS_ACCESS_KEY_ID",
            base_url_env: "AWS_BASE_URL",
            default_base_url: openai_compat::DEFAULT_AWS_BASE_URL,
        });
    }
    None
}
...
pub fn detect_provider_kind(model: &str) -> ProviderKind {
    ...
    if openai_compat::has_api_key("AI21_API_KEY") {
        return ProviderKind::AI21;
    }
    // Ollama (and any other OpenAI-compatible local server) — when
    // OLLAMA_BASE_URL is set, route *any* model name to it so users can
    // pass bare model names like "llama3" or "codellama:7b" without a prefix.
...
pub fn model_token_limit(model: &str) -> Option<ModelTokenLimit> {
    ...
    match canonical.as_str() {
        ...
        "groq" | "groq/lama-3.3-70b-versatile" => Some(ModelTokenLimit {
            max_output_tokens: 8_192,
            context_window_tokens: 128_000,
        }),
        // DeepSeek models
        // Source: https://api.deepseek.com/docs
        "deepseek-chat" | "deepseek-coder" => Some(ModelTokenLimit {
            max_output_tokens: 64_000,
            context_window_tokens: 640_000,
        }),
        // Perplexity Sonar models
        // Source: https://docs.perplexity.ai
        "sonar" | "sonar-pro" | "sonar-reasoning" => Some(ModelTokenLimit {
            max_output_tokens: 64_000,
            context_window_tokens: 128_000,
        }),
        // Mistral models
        // Source: https://docs.mistral.ai
        "mistral-large-2411" | "mistral-small-2501" => Some(ModelTokenLimit {
            max_output_tokens: 32_000,
            context_window_tokens: 128_000,
        }),
        // Cohere models
...
const FOREIGN_PROVIDER_ENV_VARS: &[(&str, &str, &str)] = &[
    ...
    (
        "GROQ_API_KEY",
        "Groq",
        "prefix your model name with `groq/` (e.g. `--model groq/llama-3.3-70b-versatile`) so prefix routing selects the Groq backend",
    ),
    (
        "PERPLEXITY_API_KEY",
        "Perplexity",
        "prefix your model name with `perplexity/` (e.g. `--model perplexity/sonar`) so prefix routing selects the Perplexity backend",
    ),
    (
        "DEEPSEEK_API_KEY",
        "DeepSeek",
    ...
    (
        "MISTRAL_API_KEY",
        "Mistral",
        "prefix your model name with `mistral/` (e.g. `--model mistral/mistral-large-2411`) so prefix routing selects the Mistral backend",
    ),
    (
        "COHERE_API_KEY",
        "Cohere",
        "prefix your model name with `cohere/` (e.g. `--model cohere/command-r-plus`) so prefix routing selects the Cohere backend",
    ),
    (
        "OPENROUTER_API_KEY",
        "OpenRouter",
        "prefix your model name with `openrouter/` (e.g. `--model openrouter/anthropic/claude-sonnet-4`) so prefix routing selects the OpenRouter backend",
    ),
    (
        "TOGETHER_API_KEY",
        "Together AI",
        "prefix your model name with `together/` (e.g. `--model together/llama-4-scout`) so prefix routing selects the Together backend",
    ),
    (
        "AI21_API_KEY",
        "AI21",
        "prefix your model name with `ai21/` (e.g. `--model ai21/jamba-1.5-large`) so prefix routing selects the AI21 backend",
    ),
];
