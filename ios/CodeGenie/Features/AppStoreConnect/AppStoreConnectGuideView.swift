...
struct AppStoreConnectGuideView: View {
    ...
    /// Three-chip legend explaining the Auto / Hybrid / You badges on
    /// each step. Saves the user from guessing which steps they have
    /// to do versus which CodeGenie handles silently.
...
private struct ASCStepCard: View {
    ...
    /// Chip next to the step number flagging who actually does the
    /// work — CodeGenie, the user, or both. Closes the audit gap where
    /// the 10-step list mixed automated and user actions silently.
